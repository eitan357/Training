...
Find the `Object.assign(window, { ... })` block containing running exports (search `grep -n "runShowDashboard\|runShowAdd\|runShowHistory\|runShowStep1\|runSelectType\|runHandleOcr\|runSaveWorkout\|runGoBack\|runToggleHistoryRow" public/index.html` for its exact current location) and remove every deleted function's name from the list. Keep `runSetRange` (still live, per Step 4).

**Plan defect, fixed after Task 5's own review caught it in practice:** an earlier version of this step also told you to add `selectCardioType`/`copyLastCardioWorkout`/`clearCardioForm`/`addCustomCardioField`/`submitCardioData` here — Task 5's implementer discovered live (by actually opening the cardio page in a browser) that these 5 were never exported anywhere, meaning every button on the cardio page threw `ReferenceError` from the moment Task 4 landed until this task — a real gap spanning 4 intervening tasks. It fixed it immediately rather than staying blocked, so **these 5 are already exported as of Task 5's commit — do not add them again** (duplicate object-literal keys are legal JS and harmless, but pointless). Only add the Task 6 cardio-editor functions here, once Task 6 has actually landed: `openCardioEdit`, `closeCardioEdit`, `switchCardioEditTab`, `addCardioEditField`, `removeCardioEditField`, `setCardioFieldType`, `confirmAddCardioType`, `removeCardioType`, `saveCardioTemplates`. Confirm via `grep -n "selectCardioType\|copyLastCardioWorkout\|clearCardioForm\|addCustomCardioField\|submitCardioData" public/index.html` that these 5 already appear inside an `Object.assign(window, {...})` block before assuming you need to add them.
