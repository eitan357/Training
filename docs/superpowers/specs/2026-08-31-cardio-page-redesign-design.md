...
2. ~~The per-type color scheme...~~ **Resolved:** strength types are *also* open-ended (existing template editor already allows a 3rd+ type, which today silently renders with no color — a pre-existing bug). Fix: a shared 6-color palette indexed by a type's position in its domain's `types` array (`typeColorClass(types, typeName)`, introduced in `2026-08-31-A-shared-workout-engine.md` Task 7), used identically by both domains. Index 0/1 preserve A's/B's exact current green/purple.
