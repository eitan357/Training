...
**Known limitation, explicitly accepted by Eitan:** none of the three default sets above live-retranslate if the user later switches app language — they're written once, in whichever language was detected at account-creation time, and stay that way until the user manually edits them (identical behavior to how the strength/measurement defaults already worked before this change, and consistent with how a user-renamed cardio field already behaves today).
